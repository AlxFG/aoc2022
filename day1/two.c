...
int32_t get_largest(list_t *node, int32_t elves[]);
int8_t in_array(int32_t array[], int32_t value);
int32_t sum_array(int32_t array[]);
...
int32_t
get_largest(list_t *node, int32_t elves[])
{
	for (int i = 0; i < 3; i++) {
		int32_t largest = 0;
		list_t *tmp = node;
		while (tmp) {
			if (tmp->data > largest && !in_array(elves, tmp->data))
				largest = tmp->data;
			tmp = tmp->next;
		}
		elves[i] = largest;
	...
	return sum_array(elves);
}

int8_t
in_array(int32_t array[], int32_t value)
{
	if (array[0] == value)
		return 1;
